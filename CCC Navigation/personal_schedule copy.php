...
if(!isset($_POST['syncButton'])) {
    if (isset($_GET['day']) && $_GET['day'] != 'ALL') {
        ...
        $sql = "SELECT * FROM schedule WHERE day = '$selected_day' AND user_id = $userid ";
      } else {
    ...
    if (isset($_GET['day']) && $_GET['day'] != 'ALL') {
        // If a specific day is selected, filter schedules by that day
        $selected_day = $_GET['day'];
        $sql = "SELECT * FROM schedule WHERE day = '$selected_day' and user_id = $userid AND section IS NULL";
    } else {
        // If 'ALL' is selected or no day is selected, fetch all schedules
        $sql = "SELECT * FROM schedule WHERE user_id = $userid AND section IS NULL";
    }
}
...
<script>
  ...
  //for toggle button sync
  document.addEventListener('DOMContentLoaded', function() {
        // Get reference to the sync button
        var syncButton = document.getElementById('syncButton');

        // Initial state: false means the 'if' part will be executed first
        var buttonState = false;

        // Add event listener to toggle button behavior
        syncButton.addEventListener('click', function() {
            if (buttonState) {
                // 'else' part: If the buttonState is true, execute this block
                // Execute 'else' logic here
                console.log("Else part executed");
            } else {
                // 'if' part: If the buttonState is false, execute this block
                // Execute 'if' logic here
                console.log("If part executed");
            }
            
            // Toggle buttonState for the next click
