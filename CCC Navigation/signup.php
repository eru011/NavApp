...
if (isset($_POST['signup'])) {
    ...
    $pass = $_POST['password'];
    $email = $_POST['email'];
    $section = $_POST['section']; // New section input
    ...
    $sql = "SELECT * FROM users WHERE email = '$email'"; //Checking if email already exists
    ...
    if (count($errors) > 0) {
        ...
        $sql = "INSERT INTO users (username, email, password, section) VALUES (?, ?, ?, ?)";
        $stmt = mysqli_stmt_init($conn);
        $preparestmt = mysqli_stmt_prepare($stmt, $sql);

        if ($preparestmt) {
            mysqli_stmt_bind_param($stmt, "ssss", $user_name, $email, $pass_hash, $section);
            mysqli_stmt_execute($stmt);

            echo "<div class='alert alert-success'>Successfully Registered!</div>";
            header("Location: login.php");
        } else {
            die("Something went wrong with the user insertion");
        }
    }
}
?>
    ...
    <div class="box">
        <form action="signup.php" method="post"><!--For the back end to work need ng form tag-->
            ...
            <div class="input-group">
                ...
                <input type="text" id="section" name="section" alt="text" placeholder="Input your section" required>
            </div>
            <div class="input-group">
